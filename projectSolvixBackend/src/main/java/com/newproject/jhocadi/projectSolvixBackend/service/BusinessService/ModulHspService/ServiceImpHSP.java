package com.newproject.jhocadi.projectSolvixBackend.service.BusinessService.ModulHspService;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.newproject.jhocadi.projectSolvixBackend.dtos.BusinessDtos.ModulHSPDtos.RequestHSP;
import com.newproject.jhocadi.projectSolvixBackend.dtos.BusinessDtos.ModulHSPDtos.ResponseHSP;
import com.newproject.jhocadi.projectSolvixBackend.dtos.BusinessDtos.ModulHSPDtos.ResponseHSPMes;
...
import com.newproject.jhocadi.projectSolvixBackend.repository.BusinessRepo.ModulHspRepo.RepositoryHSP;
...
public class ServiceImpHSP implements ServiceHSP {
    ...
    // ── HSP crítica, favorable, promedio, diseño ──────────────────────────
        ResponseHSPMes critico   = meses.stream()
            ...
            .min(Comparator.comparingDouble(m -> m.getHspDiaria())).orElseThrow();
        ResponseHSPMes favorable = meses.stream()
            .filter(Objects::nonNull)
            .max(Comparator.comparingDouble(m -> m.getHspDiaria())).orElseThrow();

    double sumaHOptM   = meses.stream().filter(Objects::nonNull).mapToDouble(m -> m.getHOptM()).sum();
