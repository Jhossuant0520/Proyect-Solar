...
import com.newproject.jhocadi.projectSolvixBackend.repository.BusinessRepo.ModulDemandaReciboRepo.RepositoryDemandaRecibo;
...
public class ServiceImpDemandaRecibo implements ServiceDemandaRecibo {
    ...
    private ResponseDemandaRecibo calcular(RequestDemandaRecibo request) {
        ...
        if ("RECIBOS".equalsIgnoreCase(request.getModoCalculoConsumoBase())) {
            ...
            double sum = 0.0;
            int count = 0;
            for (Double v : recibos) {
                if (v == null) continue;
                sum += v.doubleValue();
                count++;
            }
            if (count == 0)
                throw new IllegalArgumentException("Debe ingresar valores válidos en los recibos.");
            consumoBaseKwh = sum / count;
            response.setCantidadRecibosUsados(count);
